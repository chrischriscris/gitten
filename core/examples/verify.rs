...
fn main() {
    ...
    for (i, c) in commits.iter().enumerate() {
        ...
        for p in c.parents.iter().skip(1) {
            if let Some(&j) = index.get(p.as_str()) {
                if rows[j].lane == rows[i].lane
                    && children.get(p.as_str()).copied().unwrap_or(0) == 1
                {
    ...
    println!(
        "   same lane        {inherit_ok}  ({:.2}%)",
        inherit_ok as f64 / checked as f64 * 100.0
    );
    println!("   shared parent    {inherit_shared}  (legit: parent has >1 child)");
    println!("   UNEXPLAINED      {inherit_bad}");
    println!();
    println!(
        "merge second parents on the merge's own lane (should be 0): {second_parent_same_lane}"
    );
    ...
    println!(
        "\nOK — lane contract holds across {} commits",
        commits.len()
    );
}
